...
public class uciTest {
	...
	public static List<List<Double>> read_wine(String path) {
        ...
        try {  
            ...
            while ((line=br.readLine()) != null) {  
                String strArray[] = line.split("\t");  
				data = new ArrayList<Double>();
				category = Double.parseDouble(strArray[0]);
				//data.add(category - 1);
				data.add(category);
	 ...
	 * softmax进行训练及测试
	 */
	public static void process(String trainPath,String testPath,int n_in) {
		/*
	...
	public static void main(String[] args) {
		...
		String trainPath = dir + args[1];  //训练数据集路径路径,文件必须为第一列为标签数据,剩余列为x数据
		String testPath = dir + args[2];   //测试数据集路径,文件必须为第一列为标签数据,剩余列为x数据
		int n = Integer.parseInt(args[3]); //输入维度
		process(trainPath,testPath,n);
	}
